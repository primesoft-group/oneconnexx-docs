...
| | |
| __parameter__ | |
| sourceProtocol | The protocol to use to read the source files. The choices are: NULL, FILE, FTP, FTPS, SFTP, S3, WebDAV, SHAREPOINT and GRAPHAPI |
...
| sourceTimeFilter | FILE, SFTP, SHAREPOINT, GRAPHAPI: Time filter to only process files that are older than the set time (in minutes) |
